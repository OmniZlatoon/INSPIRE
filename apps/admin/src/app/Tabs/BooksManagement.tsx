...
const SkeletonCard = () => (
    <div className="bg-white dark:bg-[#1a1a1a] border border-gray-100 dark:border-[#2E2E2E] rounded-xl p-4 animate-pulse">
        ...
        <div className="h-4 w-3/4 bg-gray-200 dark:bg-gray-800 rounded mb-1"></div>
        <div className="h-3 w-1/4 bg-gray-100 dark:bg-gray-800 rounded mb-2"></div>
        <div className="h-3 w-1/3 bg-gray-100 dark:bg-gray-800 rounded mb-3"></div>
        <div className="flex items-center justify-between pt-2.5 border-t border-gray-50 dark:border-[#2E2E2E]">
            <div className="h-3 w-1/2 bg-gray-200 dark:bg-gray-800 rounded"></div>
            <div className="h-3 w-6 bg-gray-200 dark:bg-gray-800 rounded"></div>
        </div>
    </div>
);
...
export default function BooksManagement() {
    ...
    const ContextMenu = ({ book }: { book: Book }) => (
        <div className="book-menu relative">
            ...
            {activeMenu === book.id && (
                <div className="absolute right-0 top-8 w-44 bg-white dark:bg-[#2d2d2d] border border-gray-100 dark:border-[#2E2E2E] rounded-xl shadow-xl z-50 py-1">
    ...
    return (
        <div className="p-8 w-full min-h-full relative animate-in fade-in slide-in-from-bottom-4 duration-500">
            {/* Toast */}
            {toast && (
                <div className={`fixed bottom-6 right-6 z-[200] flex items-center gap-2.5 px-4 py-2.5 bg-white dark:bg-[#1e1e1e] border border-green-200 dark:border-[#2E2E2E] rounded-lg shadow-lg transition-all duration-500 ${exiting ? 'opacity-0 translate-x-4' : 'opacity-100 translate-x-0'}`}>
                        ...
                        viewMode === 'grid' ? (
                            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
                                {filterBooks(books, searchQuery).map(book => (
                                    <div key={book.id} className="bg-white dark:bg-[#1a1a1a] border border-gray-100 dark:border-[#2E2E2E] hover:border-gray-200 dark:hover:border-gray-700 rounded-xl p-4 group transition-all duration-200">
                                        <div className="flex justify-between items-start mb-3">
