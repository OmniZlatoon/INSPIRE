...
export function CourseDropdown({ courses, selected, onSelect }: { courses: Course[]; selected: string; onSelect: (id: string) => void }) {
    ...
    return (
        <div className="relative" ref={ref}>
            <button type="button" onClick={() => setOpen(!open)} className="w-full flex justify-between items-center px-4 py-3 bg-gray-50 dark:bg-[#121212] border border-gray-200 dark:border-[#2E2E2E] rounded-xl text-sm text-[#202124] dark:text-white hover:border-primary transition-colors">
            ...
            {open && (
                <div className="absolute top-full left-0 right-0 mt-1 bg-white dark:bg-[#1e1e1e] border border-gray-100 dark:border-[#2E2E2E] rounded-xl shadow-2xl z-50 max-h-52 overflow-y-auto py-1">
    ...
    return (
        <div>
            ...
            <div onDragOver={e => e.preventDefault()} onDrop={e => { e.preventDefault(); handleFiles(e.dataTransfer.files); }}
                className="border-2 border-dashed border-gray-200 dark:border-[#2E2E2E] rounded-xl p-4 text-center cursor-pointer hover:border-primary transition-colors"
            ...
            {files.length > 0 && (
                <div className="mt-2 space-y-1.5">
                    {files.map((f, i) => (
                        <div key={i} className="flex items-center justify-between px-3 py-2 bg-gray-50 dark:bg-[#121212] rounded-lg border border-gray-100 dark:border-[#2E2E2E]">
...
export function SingleBookForm({ form, onChange, courses }: { form: BookForm; onChange: (f: BookForm) => void; courses: Course[] }) {
    const inp = "w-full px-4 py-3 bg-gray-50 dark:bg-[#121212] border border-gray-200 dark:border-[#2E2E2E] rounded-xl focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none text-sm text-[#202124] dark:text-white dark:caret-white transition-all placeholder:text-gray-400 dark:placeholder:text-gray-600";
    return (
...
export function BulkBookForm({ forms, setForms, courses }: { forms: BulkBookEntry[]; setForms: (f: BulkBookEntry[]) => void; courses: Course[] }) {
    const inp = "w-full px-4 py-2.5 bg-white dark:bg-[#1e1e1e] border border-gray-200 dark:border-[#2E2E2E] rounded-lg outline-none text-sm text-[#202124] dark:text-white dark:caret-white placeholder:text-gray-400 dark:placeholder:text-gray-600";
    const upd = (idx: number, field: keyof BulkBookEntry, val: any) => setForms(forms.map((f, i) => i !== idx ? f : { ...f, [field]: val }));
    return (
        <div className="space-y-4">
            {forms.map((form, idx) => (
                <div key={idx} className="p-5 border border-gray-100 dark:border-[#2E2E2E] rounded-2xl bg-gray-50/30 dark:bg-[#121212]/30 space-y-3 relative">
            ...
            <button onClick={() => setForms([...forms, { bookId: '', name: '', description: '', author: '', courseId: '', files: [] }])} className="w-full py-3 border-2 border-dashed border-gray-200 dark:border-[#2E2E2E] rounded-2xl text-[#5f6368] hover:border-primary hover:text-primary flex items-center justify-center gap-2 font-medium transition-all">
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/50 backdrop-blur-md">
            <div className="bg-white dark:bg-[#1a1a1a] rounded-2xl w-full max-w-2xl shadow-2xl flex flex-col max-h-[90vh]">
                <div className="flex justify-between items-center px-6 py-4 border-b border-gray-100 dark:border-[#2E2E2E] bg-gray-50/50 dark:bg-[#111]/50">
                ...
                <div className="p-6 border-t border-gray-100 dark:border-[#2E2E2E] relative overflow-hidden">
                    {isLoading && (
                    ...
                    {error && <div className="flex items-center gap-2 p-3 mb-3 bg-red-50 dark:bg-red-900/20 text-red-600 rounded-lg text-xs border border-red-100 dark:border-[#2E2E2E]"><AlertTriangle size={14} />{error}</div>}
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
            <div className="bg-white dark:bg-[#1a1a1a] rounded-2xl w-full max-w-md shadow-2xl p-8">
                ...
                <div className="grid grid-cols-2 gap-3">
                    <button onClick={onClose} className="py-2.5 rounded-xl font-semibold text-[#5f6368] border border-gray-200 dark:border-[#2E2E2E] hover:bg-gray-50 dark:hover:bg-[#2d2d2d]">Cancel</button>
                ...
                <div className="flex items-center justify-between px-8 py-5 border-b border-gray-100 dark:border-[#2E2E2E] bg-white dark:bg-[#1a1a1a] flex-shrink-0">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p className="text-sm text-[#5f6368] font-sans mt-0.5">{book.bookId}</p>
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 divide-y lg:divide-y-0 lg:divide-x divide-gray-100 dark:divide-gray-800 min-h-full">
                            ...
                            {book.files && book.files.length > 0 && (
                                <section>
                                    ...
                                    <div className="space-y-2">
                                        {book.files.map((f, i) => (
                                            <div key={i} className="flex items-center gap-3 px-3 py-2 bg-gray-50 dark:bg-[#1a1a1a] border border-gray-100 dark:border-[#2E2E2E] rounded-lg">
                                                {fileIcon(f.type)}
                        ...
                        <div className="p-8 space-y-10 bg-gray-50/40 dark:bg-[#0d0d0d]">
                            ...
                            <section>
                                ...
                                {course ? (
                                    <span className="inline-block text-xs px-3 py-1.5 bg-[#e8f0fe] dark:bg-blue-900/20 text-[#1a73e8] dark:text-blue-400 rounded-lg border border-gray-200 dark:border-[#2E2E2E] font-semibold">{course.name}</span>
                                ) : (
